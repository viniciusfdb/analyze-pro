...
import '../../../../repositories/vendas/estruturamercadologica/vendas_por_vendedor_subgrupo_repository.dart';
import '../../../../services/auth_service.dart';
import '../../../../services/cad_lojas_service.dart';
import '../../../../models/cadastros/cad_lojas.dart';
import 'vendas_por_vendedor_produto_page.dart';

class VendasPorVendedorSubgrupoPage extends StatefulWidget {
  final int idEmpresa;
  ...
  const VendasPorVendedorSubgrupoPage({
    Key? key,
  ...
  @override
  State<VendasPorVendedorSubgrupoPage> createState() => _VendasPorVendedorSubgrupoPageState();
}

class _VendasPorVendedorSubgrupoPageState extends State<VendasPorVendedorSubgrupoPage> {
  ...
  late final VendasPorVendedorSubgrupoRepository _repository;
  late final CadLojasService _cadLojasService;

  List<Empresa> _empresas = [];
  List<Empresa> _empresasSelecionadas = [];
  DateTimeRange? _selectedDateRange;
  List<VendaPorVendedorSubgrupoModel> _resultados = [];
  bool _loading = true;
  ...
  void initState() {
    ...
    _repository = VendasPorVendedorSubgrupoRepository(_apiClient);
  ...
  Future<void> _carregarDados() async {
    ...
    final List<VendaPorVendedorSubgrupoModel> resultados = [];

    for (final empresa in _empresasSelecionadas) {
      final dados = await _repository.getVendasPorVendedorSubgrupo(
          ...
          children: [
            if (_empresas.isNotEmpty)
              PopupMenuButton<Empresa>(
                color: Colors.white,
                itemBuilder: (context) => _empresas.map((empresa) {
            ...
            if (_loading)
              ...
              const Center(child: Padding(padding: EdgeInsets.only(top: 32), child: Text('Nenhum resultado encontrado.')))
            else
              ListView.builder(
                itemCount: _resultados.where((e) => e.descricao.toLowerCase().contains(_termoBusca)).length,
                shrinkWrap: true,
                  ...
                  return GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => VendasPorVendedorProdutoPage(
