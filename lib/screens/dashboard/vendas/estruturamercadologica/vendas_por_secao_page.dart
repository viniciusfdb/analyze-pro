import 'package:analyzepro/screens/dashboard/vendas/estruturamercadologica/vendas_por_grupo_page.dart';
...
import '../../../../repositories/vendas/estruturamercadologica/vendas_por_secao_repository.dart';

import 'dart:async';
import 'package:analyzepro/repositories/cronometro/tempo_execucao_repository.dart';

class VendasPorSecaoPage extends StatefulWidget {
  final List<Empresa> empresasSelecionadas;
  final DateTimeRange intervalo;
  final int? idDivisao;

  const VendasPorSecaoPage({
    super.key,
  ...
  @override
  State<VendasPorSecaoPage> createState() => _VendasPorSecaoPageState();
}

class _VendasPorSecaoPageState extends State<VendasPorSecaoPage> with WidgetsBindingObserver {
  final DateFormat _formatter = DateFormat('dd/MM/yyyy');

  late final AuthService _authService;
  late final ApiClient _apiClient;
  late final VendasPorSecaoRepository _repository;
  late final CadLojasService _cadLojasService;

  List<Empresa> _empresas = [];
  List<Empresa> _empresasSelecionadas = [];
  DateTimeRange? _selectedDateRange;
  List<VendaPorSecaoModel> _resultados = [];
  bool _loading = true;
  ...
  void initState() {
    ...
    _repository = VendasPorSecaoRepository(_apiClient);
  ...
  Future<void> _inicializarTempoExecucao() async {
    ...
    final chave =
        '${_empresasSelecionadas.map((e) => e.id).join(",")}|$dias|vendas_secao';
  ...
  Future<void> _carregarDados() async {
    ...
    final Map<int, VendaPorSecaoModel> mapaResultados = {};

    for (final empresa in _empresasSelecionadas) {
      final dados = await _repository.getVendasPorSecao(
      ...
      for (final item in dados) {
        if (mapaResultados.containsKey(item.idSecao)) {
          final acumulado = mapaResultados[item.idSecao]!;
          mapaResultados[item.idSecao] = acumulado.copyWith(
            valTotLiquido: acumulado.valTotLiquido + item.valTotLiquido,
            lucro: acumulado.lucro + item.lucro,
          );
        } else {
          mapaResultados[item.idSecao] = item;
        }
      }
    }
        ...
        child: Column(
          ...
          children: [
            ...
            if (_loading)
              ...
              ListView.builder(
                itemCount: _resultados.where((e) => e.descricao.toLowerCase().contains(_termoBusca)).toList().length,
                shrinkWrap: true,
    ...
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => VendasPorGrupoPage(
